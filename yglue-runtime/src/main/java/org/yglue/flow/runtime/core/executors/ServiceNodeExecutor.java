 ...
 * <p>脚本执行环境：</p>
 * <ul>
 * <li>{@code ctx} - 流程上下文数据（Map），可通过 {@code ctx.request.path.xxx}、{@code
 * ctx['key']} 等方式访问</li>
 * </ul>
 * 
 * @author yglue
...
public class ServiceNodeExecutor implements FlowExecutor {
    ...
    private static final ConcurrentHashMap<String, Map<String, Object>> CONFIG_JSON_CACHE = new ConcurrentHashMap<>();
    private static final ConcurrentHashMap<MethodCacheKey, Method> METHOD_CACHE = new ConcurrentHashMap<>();
    private static final Class<?> NULL_ARG_CLASS = NullArgMarker.class;
    ...
    public Object execute(NodeExecutionContext context) throws Exception {
        ...
        log.debug("[ServiceNodeExecutor] 开始执行任务节点: nodeId={}", nodeId);
        ...
        Map<String, Object> configJson = parseConfigJson(comp);

        // 获取 bean 名称和方法名
        String beanName = getBeanName(comp, configJson);
        String methodName = getMethodName(comp, configJson);
        log.debug("[ServiceNodeExecutor] 解析组件信息: nodeId={}, beanName={}, methodName={}", nodeId, beanName, methodName);
        ...
        try {
            ...
            log.debug("[ServiceNodeExecutor] 成功获取 bean: nodeId={}, beanName={}, beanClass={}, beanType={}",
                    nodeId, beanName,
        ...
        // 解析输入参数 - 优先检查 V2 结构化拦截器是否已解析
        log.debug("[ServiceNodeExecutor] 开始解析输入参数: nodeId={}", nodeId);
        List<Object> arguments = new ArrayList<>();
        List<Object> resolvedArgs = (List<Object>) config.get("_resolvedArgs");

        if (resolvedArgs != null) {
            log.debug("[ServiceNodeExecutor] 检测到 V2 无脚本预解析参数: nodeId={}, count={}", nodeId, resolvedArgs.size());
            arguments.addAll(resolvedArgs);
        } else {
        ...
        log.debug("[ServiceNodeExecutor] 输入参数解析完成: nodeId={}, argumentCount={}", nodeId, arguments.size());
        ...
        log.debug("[ServiceNodeExecutor] 找到方法: nodeId={}, method={}", nodeId, method);
        ...
        log.debug("[ServiceNodeExecutor] 开始调用方法: nodeId={}, beanName={}, method={}",
                nodeId, beanName, method.getName());
        Object result;
        try {
            result = method.invoke(bean, convertedArgs);
            log.debug("[ServiceNodeExecutor] 方法调用成功: nodeId={}, beanName={}, method={}, resultType={}",
        ...
        log.debug("[ServiceNodeExecutor] 任务节点执行完成: nodeId={}, beanName={}, method={}",
                nodeId, beanName, methodName);
        return result;
    }

    @SuppressWarnings("unchecked")
    private Map<String, Object> parseConfigJson(Map<String, Object> comp) {
        ...
        if (configJsonObj instanceof String configJsonStr) {
            String trimmed = configJsonStr.trim();
            if (trimmed.isEmpty()) {
                return null;
            }
            return CONFIG_JSON_CACHE.computeIfAbsent(trimmed, key -> {
                try {
                    Map<String, Object> parsed = objectMapper.readValue(key, Map.class);
                    return Collections.unmodifiableMap(parsed);
                } catch (Exception ex) {
                    throw new IllegalArgumentException("Failed to parse comp.configJson", ex);
                }
            });
        }

        return objectMapper.convertValue(configJsonObj, Map.class);
    }
     ...
     */
    private String getMethodName(Map<String, Object> comp, Map<String, Object> configJson) {
     ...
     */
    private String getBeanName(Map<String, Object> comp, Map<String, Object> configJson) {
    ...
    private Method resolveMethod(Class<?> clazz, String name, java.util.List<Object> args) {
        MethodCacheKey cacheKey = MethodCacheKey.of(clazz, name, args);
        ...
        for (Method m : candidates) {
            ...
            if (best == null || score < bestScore
                    || (score == bestScore && java.lang.reflect.Modifier.isPublic(m.getModifiers())
                            && !java.lang.reflect.Modifier.isPublic(best.getModifiers()))) {
                best = m;
                bestScore = score;
            }
        }
        if (best != null) {
            ...
            METHOD_CACHE.putIfAbsent(cacheKey, best);
            return best;
        }
        // 未找到方法，收集信息便于调试
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            if (method.getName().equals(name)
                    && !availableMethods.contains(method.getName() + "/" + method.getParameterCount())) {
                availableMethods.add(method.getName() + "/" + method.getParameterCount() +
                        " (" + java.util.Arrays.toString(method.getParameterTypes()) + ")");
            }
        }
    ...
    private boolean canConvertTo(Class<?> targetType, Object value) {
        ...
        try {
            ...
            return targetType.isInstance(converted) || converted.getClass().equals(targetType)
                    || (targetType == String.class && converted instanceof String);
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private Object convert(Object value, Class<?> targetType) {
        ...
        if (targetType.isEnum()) {
            ...
            Object enumValue = java.lang.Enum
                    .valueOf((Class<? extends java.lang.Enum>) targetType.asSubclass(java.lang.Enum.class), name);
            return enumValue;
        }
        // Map / JSON-string → POJO via ObjectMapper
        if (value instanceof java.util.Map) {
            try {
                return objectMapper.convertValue(value, targetType);
            } catch (IllegalArgumentException e) {
                // 提取更详细的字段错误信息，帮助排查哪个字段类型不匹配
                String detail = e.getCause() != null ? e.getCause().getMessage() : e.getMessage();
                throw new IllegalArgumentException(
                        String.format("无法将 Map 构造为 [%s]：%s（原始值：%s）",
                                targetType.getSimpleName(), detail, value),
                        e);
            }
        }
        // 兜底：如果入参是 JSON 字符串，尝试先反序列化为 Map 再转换为 POJO
        if (value instanceof String strVal && !targetType.equals(String.class)) {
            ...
            if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
                try {
                    ...
                    throw new IllegalArgumentException(
                            String.format("无法将 JSON 字符串构造为 [%s]：%s",
    ...
    private Object resolveInputValue(Map<String, Object> input,
        ...
        // 统一通过 JsonPath 或 SpEL 脚本执行获取值，替代原先沉重的 Groovy 引擎
        try {
            if (script.startsWith("ctx.")) {
                // 常见路径属性访问直接降级为高速 JsonPath
                String path = script.substring(4);
                Object result = JsonPathUtil.extract(flowContext.data(), path);
                log.debug("[ServiceNodeExecutor] JsonPath 执行结果 [{}]: nodeId={}, result={}, resultType={}",
                ...
                spelCtx.setVariables(flowContext.data());
                spelCtx.setVariable("ctx", flowContext.data());
                Object result = SPEL_PARSER.parseExpression(script).getValue(spelCtx);
                log.debug("[ServiceNodeExecutor] SpEL 脚本执行结果 [{}]: nodeId={}, result={}, resultType={}",
                        index, nodeId, result, result != null ? result.getClass().getName() : "null");
                return result;
            }
        } catch (Exception e) {
            log.error("[ServiceNodeExecutor] 参数表达式执行失败 [{}]: nodeId={}, script={}, error={}",
                    index, nodeId, script, e.getMessage(), e);

            throw new RuntimeException("Failed to evaluate script/path for input parameter [" + index + "]: "
                    + e.getMessage() + "\n表达式内容: " + script, e);
        }
    }
    ...
    private void validateInput(Object value,
        ...
        if ("OBJECT".equals(valueType) && typeName != null && !typeName.isBlank() && rules.isEmpty()) {
            ...
            List<BeanValidationExtractor.ObjectFieldValidation> beanValidations = BeanValidationExtractor
                    .extractFieldValidations(typeName);
        ...
        if (!validationErrors.isEmpty()) {
            ...
            if ("throw".equals(failPolicy)) {
                ...
                log.debug("[ServiceNodeExecutor] 参数校验失败，跳过节点: nodeId={}, message={}",
                        nodeId, message);
    ...
    private void validateObjectFields(Object object,
        ...
        if (!validationErrors.isEmpty()) {
            ...
            if ("throw".equals(failPolicy)) {
                ...
                log.debug("[ServiceNodeExecutor] 对象字段校验失败，跳过节点: nodeId={}, message={}",
                        nodeId, message);
    ...
    private static final class NullArgMarker {
        private NullArgMarker() {
        }
    }

    private record MethodCacheKey(Class<?> targetClass, String methodName, List<Class<?>> argTypes) {
        private static MethodCacheKey of(Class<?> targetClass, String methodName, List<Object> args) {
            List<Class<?>> signature = new ArrayList<>(args.size());
            for (Object arg : args) {
                signature.add(arg == null ? NULL_ARG_CLASS : arg.getClass());
            }
            return new MethodCacheKey(targetClass, methodName, List.copyOf(signature));
        }
    }
}
