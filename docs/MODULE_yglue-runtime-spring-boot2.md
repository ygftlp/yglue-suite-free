# yglue-runtime-spring-boot2 模块文档

## 模块概述

`yglue-runtime-spring-boot2` 是 YGFlow Suite 的 Spring Boot 2.x 集成模块，提供了将运行时引擎集成到 Spring Boot 2.x 应用中的自动配置和 AOP 拦截能力。

## 模块职责

- **Spring Boot 2 自动配置**：提供 Spring Boot 2.x 的自动配置类
...
### 被依赖场景
- Spring Boot 2.x 业务应用集成运行时引擎

## 使用场景

1. **业务服务集成**：在 Spring Boot 2.x 业务服务中集成流程编排能力
...
- **打包方式**：JAR
- **Maven 坐标**：`org.yg:yglue-runtime-spring-boot2:0.1.0-SNAPSHOT`
- **源码打包**：自动生成 `-sources.jar` 文件

## 版本兼容性

- **Java 版本**：Java 17+
- **Spring Boot 版本**：2.7.x - 2.9.x
- **Spring 版本**：5.3.x - 5.4.x
